...
export function MobileNav() {
  return (
    <nav className="fixed inset-x-0 bottom-0 z-20 md:hidden border-t border-subtle bg-surface/85 backdrop-blur-xl shadow-[0_-20px_60px_-35px_rgba(0,0,0,0.65)]">
      <div className="mx-auto grid max-w-md grid-cols-5">
        {items.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className="group flex h-16 flex-col items-center justify-center gap-1 text-[0.72rem] text-muted-foreground transition hover:text-foreground"
          >
